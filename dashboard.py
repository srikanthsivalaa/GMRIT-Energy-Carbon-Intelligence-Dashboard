...
- **Recommended framing for viva**: present the campus audit total as a *reference envelope* for context, and the Block 3 ML estimate as a separate, independently-calibrated scenario.
...
campus_solar_generation_kwh = 867317 + 261417  # 1,128,734 kWh/yr
...
cards = [
    ...
    (c2, "Estimated Block 3 Solar Allocation", f"{block3_solar_offset_kwh:,.0f} kWh", "Based on 11.15% connected-load share" if show_solar else "Disabled"),
    (c3, "Estimated CO2 Avoided (Solar)", f"{solar_co2_avoided:,.1f} tCO2", "Based on allocated solar share"),
    (c4, "Campus DG Diesel Reference", f"{campus_diesel_co2:,.1f} tCO2", f"{annual_diesel_liters:,.0f} L/yr campus-level (not allocated to Block 3)"),
    (c5, "Block 3 Electricity Carbon Footprint", f"{total_co2_block3_electricity:,.1f} tCO2/yr", "Net grid (post-solar allocation)"),
]
...
score_cards = [
    (s1, "Estimated Gross Electricity Intensity", f"{eui_gross:.1f} kWh/m²/yr", "Gross ML demand / area"),
    (s2, "Estimated Grid EUI post Solar", f"{eui_net:.1f} kWh/m²/yr", ecbc_rating),
    (s3, "Electricity Carbon Intensity", f"{carbon_intensity:.1f} kgCO2/m²/yr", "Electricity emissions / area"),
    (s4, "Built-up Area (gbXML/BIM)", f"{block3_sqm:,.0f} m²", f"{block3_floors_desc}, built {block3_yearbuilt}"),
    (s5, "ECBC / BEE Reference EUI Range", f"{ecbc_benchmark_best}-{ecbc_benchmark_normal} kWh/m²/yr", "Warm & Humid daytime institutional"),
]
for col, label, val, sub in score_cards:
    with col:
        st.markdown(f"<div class='metric-card'><div class='metric-label'>{label}</div><div class='metric-value'>{val}</div><div class='metric-sub'>{sub}</div></div>", unsafe_allow_html=True)

st.markdown("<p class='note-text'>Reference EUI Comparison against ECBC/BEE ranges, not formal code certification. Audit period: Apr 2021–Mar 2022.</p>", unsafe_allow_html=True)
...
with col_gauge:
    fig_gauge = go.Figure(go.Indicator(
        ...
        title={'text': "NMBE (%) — calibration consistency", 'font': {'color': '#E5E9F0', 'size': 12}},
...
with col_compare:
    ...
    fig_compare.update_layout(
        title="Predicted vs Audited (1 Week Anchor)", template='plotly_dark', height=280, barmode='group',
...
with col_floor1:
    ...
    fig_floor.update_layout(
        title="Estimated Weekly Electricity Contribution by Floor (kWh)", template='plotly_dark', height=320,
...
with col_floor2:
    st.markdown("""<div class='metric-card'>
        ...
        <p class='desc-text' style='margin-top:12px;'>Floor split is derived from the room-level equipment inventory schedule. Major ground-floor loads: Central UPS banks (36 kW, 54 kW), substation transformers, and Electrical Machines lab motors.</p>
    </div>""", unsafe_allow_html=True)
...
fig1.update_layout(
    title="Model-Estimated Monthly Electricity Consumption (kWh)", template='plotly_dark', height=340,
    plot_bgcolor='#171B26', paper_bgcolor='rgba(0,0,0,0)', font_color='#E5E9F0'
)
...
with col_b:
    ...
    fig3 = go.Figure(go.Bar(x=years_proj, y=projected_kwh, marker_color='#60A5FA', text=[f"{v/1000:,.1f} MWh" for v in projected_kwh], textposition='auto'))
    fig3.update_layout(
        title=f"{projection_years}-Year Growth & Climate Projection (kWh/yr)",
        template='plotly_dark',
        height=340,
        plot_bgcolor='#171B26',
        paper_bgcolor='rgba(0,0,0,0)',
